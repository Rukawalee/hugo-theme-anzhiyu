...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const adjustMenu = init => {
    ...
    if (init) {
      const blogName = document.querySelector("#blog_name > a");
      const menusEl = document.getElementById("menus");
      const blogInfoWidth = blogName ? getAllWidth(blogName.children) : 0;
      const menusWidth = menusEl ? getAllWidth(menusEl.children) : 0;
      headerContentWidth = blogInfoWidth + menusWidth;
      $nav = document.getElementById("nav");
    }

    if (!$nav) return;
    const hideMenuIndex = window.innerWidth <= 768 || ($nav && headerContentWidth > 0 && headerContentWidth > $nav.offsetWidth - 60);
  ...
  const addHighlightTool = function () {
    ...
    const $figureHighlight =
      plugin === "highlight.js" || plugin === "chroma"
  ...
  const runJustifiedGallery = function (ele) {
    ...
    const addJustifiedGallery = () => {
      ele.forEach(item => {
        ...
        if (!raw) return;
        if (item.classList.contains("url")) {
          fetchUrl(raw).then(res => {
            runJustifiedGallery(item, res);
          });
        } else {
          try {
            const data = typeof raw === "string" ? JSON.parse(raw) : raw;
            if (Array.isArray(data)) runJustifiedGallery(item, data);
          } catch (e) {
            console.warn("justifiedGallery parse failed", e);
          }
        }
      });
    };
    ...
    const jg = (GLOBAL_CONFIG.source && GLOBAL_CONFIG.source.justifiedGallery) || {};
    if (!jg.js) return;
    if (jg.css) getCSS(`${jg.css}`);
    getScript(`${jg.js}`).then(addJustifiedGallery);
  ...
  const scrollFn = function () {
    ...
    function percentageScrollFn(currentTop) {
      // 处理滚动百分比 — #percent lives only under #nav-totop (Hexo parity)
      if (!$bodyWrap || !$percentBtn) return;
      ...
      // 如果当前页面需要瀑布流，就处理瀑布流
      if (waterfallDom && typeof waterfall === "function") {
  ...
  const handleThemeChange = mode => {
    ...
    const menuDarkmodeText = $rightMenu && $rightMenu.querySelector(".menu-darkmode-text");
    if (menuDarkmodeText) {
      if (mode === "light") {
  ...
  const addPostOutdateNotice = function () {
    ...
    if (!data) return;
    // Server-side notice already present — avoid double inject.
    if (document.querySelector(".post-outdate-notice")) return;
  ...
  // Force placeholder → real src. Used after first paint and every pjax swap.
  const forceLazyImages = () => {
    document.querySelectorAll("img[data-lazy-src]").forEach(img => {
      ...
      const isPlaceholder =
        !cur ||
        cur.includes("loading.gif") ||
        cur.endsWith("data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7") ||
        cur.startsWith("data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP");
      // already showing the real asset
      if (!isPlaceholder && (cur === real || cur.endsWith(real) || real.endsWith(cur.replace(/^https?:\/\/[^/]+/, "")))) {
        img.classList.add("loaded");
        img.setAttribute("data-ll-status", "loaded");
        return;
      }
      if (isPlaceholder || img.getAttribute("data-ll-status") !== "loaded") {
        img.setAttribute("src", real);
        img.classList.add("loaded");
        img.setAttribute("data-ll-status", "loaded");
        // beat progressive opacity:0 / transition:all during class swap
  ...
  const lazyloadImg = () => {
    ...
    try {
      ...
      if (typeof window.lazyLoadInstance.loadAll === "function") {
        window.lazyLoadInstance.loadAll();
      }
    } catch (_) {}
    // sync fallback immediately — never depend only on IO
    forceLazyImages();
    requestAnimationFrame(forceLazyImages);
    window.addEventListener(
      "load",
      () => {
        forceLazyImages();
        setTimeout(forceLazyImages, 200);
      },
      { once: true }
    );
  };

  // Always recreate observer after pjax (stale instance keeps toLoadCount=0)
  window.refreshLazyImages = function () {
    // 1) force immediately — covers must never stay on loading.gif after pjax
    forceLazyImages();
    if (!(window.GLOBAL_CONFIG && GLOBAL_CONFIG.islazyload)) return;
    if (typeof LazyLoad === "undefined") return;
    // 2) update existing instance; only recreate if missing (destroy races with loadAll)
    try {
    ...
    // 3) force again after paint — wins over any async LazyLoad reset
    requestAnimationFrame(() => {
      forceLazyImages();
      setTimeout(forceLazyImages, 50);
      setTimeout(forceLazyImages, 200);
      setTimeout(forceLazyImages, 500);
      setTimeout(forceLazyImages, 1000);
    });
  };
  ...
  const listenNavMusicPause = function () {
    const timer = setInterval(() => {
      ...
      const meting = navMusicEl.querySelector("#nav-music meting-js") || navMusicEl.querySelector("meting-js");
      if (!(meting && meting.aplayer)) return;
      clearInterval(timer);
      let msgPlay = '<i class="anzhiyufont anzhiyu-icon-play"></i><span>播放音乐</span>';
      let msgPause = '<i class="anzhiyufont anzhiyu-icon-pause"></i><span>暂停音乐</span>';
      meting.aplayer.on("pause", function () {
        navMusicEl.classList.remove("playing");
        const toggle = document.getElementById("menu-music-toggle");
        if (toggle) toggle.innerHTML = msgPlay;
      ...
      meting.aplayer.on("play", function () {
        navMusicEl.classList.add("playing");
        const toggle = document.getElementById("menu-music-toggle");
        if (toggle) toggle.innerHTML = msgPause;
        const consoleMusic = document.querySelector("#consoleMusic");
  ...
  function executeShortcutKeyFunction() {
    // 是否开启快捷键
    // store as real boolean — "false" string is truthy and used to break shortcuts
    {
      const raw = localStorage.getItem("keyboardToggle");
      anzhiyu_keyboard = raw === "true" || raw === true;
    }
    ...
    function keyDownEvent(event) {
      ...
      const isKeyboardEnabled = anzhiyu_keyboard === true || anzhiyu_keyboard === "true";
      ...
      if (isKeyboardEnabled && isShiftKeyPressed && !isInInputField) {
        anzhiyu_keyUpShiftDelayEvent_timeoutId = setTimeout(() => {
          switch (event.keyCode) {
            ...
            case 83: {
              // Shift+S 站内搜索
              if (typeof window.openLocalSearch === "function") {
                const prefill = typeof selectTextNow === "string" ? selectTextNow : "";
                window.openLocalSearch(prefill);
              } else {
                const searchBtn =
  ...
  function addDarkModeEventListener(elementId, childSelector) {
    ...
    if (element && childSelector) {
      element.querySelectorAll(childSelector).forEach(childElement => {
        if (childElement.dataset.darkBound) return;
        childElement.dataset.darkBound = "1";
      ...
      if (element.dataset.darkBound) return;
      element.dataset.darkBound = "1";
  ...
  const unRefreshFn = function () {
    ...
    addDarkModeEventListener("sidebar-menus", ".darkmode_switchbutton");
  ...
  window.refreshFn = function () {
    ...
    // catch-all for any remaining darkmode switches (mobile/console duplicates)
    document.querySelectorAll(".darkmode_switchbutton").forEach(el => {
    ...
    if (pageNameEl) {
      const cfgTitle = (GLOBAL_CONFIG_SITE && GLOBAL_CONFIG_SITE.configTitle) || "";
      pageNameEl.innerText = cfgTitle ? document.title.split(` | ${cfgTitle}`)[0] : (GLOBAL_CONFIG_SITE.title || document.title);
    }
    ...
    // covers must re-hydrate after pjax swaps #body-wrap
    if (typeof window.refreshLazyImages === "function") {
      window.refreshLazyImages();
    } else if (GLOBAL_CONFIG.islazyload && window.lazyLoadInstance) {
      try {
        window.lazyLoadInstance.update();
