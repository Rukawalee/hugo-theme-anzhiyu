const anzhiyu = {
  ...
  snackbarShow: (text, showActionFunction = false, duration = 2000, actionText = false) => {
    if (!GLOBAL_CONFIG.Snackbar || typeof Snackbar === "undefined") {
      if (typeof console !== "undefined") console.debug("[snackbar]", text);
      return;
    }
  ...
  loadLightbox: ele => {
    ...
    if (!service || service === "null" || service === "undefined") return;
  ...
  initThemeColor: function () {
    ...
    if (currentTop > 26) {
      ...
      if (themeColorMeta && themeColorMeta.getAttribute("content") === themeColor) return;
  ...
  initIndexEssay: function () {
    ...
    setTimeout(() => {
      const el = document.querySelector(".essay_bar_swiper_container");
      if (!el) return;
      // prefer existing instance from bbTimeList partial (avoids double Swiper)
      if (el.swiper) {
        try {
          el.swiper.update();
          el.swiper.autoplay && el.swiper.autoplay.start();
        } catch (e) {}
        return;
      }
      el.style.height = el.style.height || "30px";
      el.style.overflow = "hidden";
      let essay_bar_swiper = new Swiper(el, {
      ...
      if (essay_bar_comtainer !== null) {
        essay_bar_comtainer.onmouseenter = function () {
          essay_bar_swiper.autoplay && essay_bar_swiper.autoplay.stop();
        };
        essay_bar_comtainer.onmouseleave = function () {
          essay_bar_swiper.autoplay && essay_bar_swiper.autoplay.start();
        };
      }
    }, 100);
  },
  ...
  reflashEssayWaterFall: function () {
    if (typeof waterfall !== "function") return;
  ...
  changeMusicBg: function (isChangeBg = true) {
    ...
    if (isChangeBg) {
      ...
      if (musiccover) anMusicBg.style.backgroundImage = musiccover.style.backgroundImage;
    } else {
      // 第一次进入，绑定事件，改背景
      let timer = setInterval(() => {
        ...
        if (musiccover) {
          ...
          if (navMeting && navMeting.aplayer && !navMeting.aplayer.audio.paused) {
            anzhiyu.musicToggle();
          }
        }
      }, 100);
      // 无播放器时停止轮询，避免空壳页常驻 timer
  ...
  getCustomPlayList: function () {
    ...
    // GLOBAL_CONFIG.navMusic is enable boolean (Hexo parity); playlist id lives on data-* or nav_music object.
    const navMusicCfg =
      (GLOBAL_CONFIG && GLOBAL_CONFIG.nav_music) ||
      (GLOBAL_CONFIG && typeof GLOBAL_CONFIG.navMusic === "object" && GLOBAL_CONFIG.navMusic) ||
      {};
    const existing = anMusicPageMeting.querySelector("meting-js");
    const userId =
      urlParams.get("id") ||
      (anMusicPage && anMusicPage.dataset.playlistId) ||
      navMusicCfg.id ||
      (existing && existing.getAttribute("id")) ||
      "";
    const userServer =
      urlParams.get("server") ||
      (anMusicPage && anMusicPage.dataset.playlistServer) ||
      navMusicCfg.server ||
      (existing && existing.getAttribute("server")) ||
      "netease";
    if (!userId) {
      // Still bind bg observer if a player was server-rendered without id rewrite.
      if (existing) anzhiyu.changeMusicBg(false);
      return;
    }
  ...
  hideTodayCard: function () {
    if (document.getElementById("todayCard")) {
      ...
      // cards were display:none; refresh lazyload so covers actually load
      if (typeof window.refreshLazyImages === "function") {
        window.refreshLazyImages();
      } else if (window.lazyLoadInstance && typeof window.lazyLoadInstance.update === "function") {
  ...
  addEventListenerMusic: function () {
    ...
    const metingEl = anMusicPage.querySelector("meting-js");
    if (!metingEl || !metingEl.aplayer) return;
    const metingAplayer = metingEl.aplayer;
  ...
  // 修改body的type类型以适配css（page-type 必须在 .js-pjax 内以便 PJAX 替换）
  setValueToBodyType: function () {
    const input = document.getElementById("page-type");
    const nextType = (input && input.value) || "anzhiyu";
    const prevType = document.body.dataset.type;
    document.body.dataset.type = nextType;
    // 音乐馆进出：清封面，并打断导航 color transition
    // #site-name 使用 transition: 0.3s !important，普通 inline 无法覆盖
    if (prevType === "music" || nextType === "music") {
      if (prevType === "music" && nextType !== "music") {
        ...
        if (menuMask) {
          menuMask.style.display = "";
          menuMask.style.animation = "";
        }
      }
      const nav = document.getElementById("nav");
      if (nav) {
        const colored = nav.querySelectorAll(
          "#site-name, #site-name .title, .back-home-button, #menus a, #nav-right a, #nav-right #toggle-menu"
        );
        colored.forEach((el) => {
          el.style.setProperty("transition", "none", "important");
        });
        // cancel in-flight color transitions, then restore stylesheet transition
        if (typeof document.getAnimations === "function") {
          document.getAnimations().forEach((anim) => {
            try {
              const target = anim.effect && anim.effect.target;
              if (target && nav.contains(target)) anim.finish();
            } catch (e) {}
          });
        }
        colored.forEach((el) => {
  ...
  keyboardToggle: function () {
    const isKeyboardOn = anzhiyu_keyboard === true || anzhiyu_keyboard === "true";

    const consoleKeyboard = document.querySelector("#consoleKeyboard");
    if (isKeyboardOn) {
      if (consoleKeyboard) consoleKeyboard.classList.remove("on");
      anzhiyu_keyboard = false;
      localStorage.setItem("keyboardToggle", "false");
    } else {
      if (consoleKeyboard) consoleKeyboard.classList.add("on");
  ...
  changeSayHelloText: function () {
    ...
    const greetings = (GLOBAL_CONFIG.authorStatus && GLOBAL_CONFIG.authorStatus.skills) || [];
...
// Hugo port: expose helpers for partials/onclick handlers
window.anzhiyu = anzhiyu;
if (typeof anzhiyuPopupManager !== "undefined") window.anzhiyuPopupManager = anzhiyuPopupManager;
