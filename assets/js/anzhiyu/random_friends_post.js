...
function fetchRandomPost() {
  ...
  if (randomPostWorking == false) {
    ...
    if (
      ...
      fetch(fetchUrl).then(r=>{if(!r.ok)throw new Error(r.status);const ct=r.headers.get("content-type")||"";if(!ct.includes("json"))throw new Error("not json");return r})
        .then(res => { if(!res.ok) throw new Error("friends fetch "+res.status); const ct=res.headers.get("content-type")||""; if(!ct.includes("application/json") && !ct.includes("+json")) throw new Error("friends fetch not json"); return res.json(); })
        .then(json => {
