(function () {
  ...
  function startAI(str, df = true) {
    ...
    if (!aiStrLength) {
      explanation.innerHTML = pageFillDescription || "暂无摘要";
      return;
    }
    ...
    try {
      ...
      if (response.status === 403) {
        ...
        try {
          ...
          result = { summary: (raw && raw.slice(0, 120)) || "AI 摘要服务暂不可用" };
        }
      }
  ...
  function aiAbstractLocal() {
    let strArr = String(postAI === true ? "" : postAI || "")
      .split(",")
      .map(item => item.trim())
      .filter(Boolean);
    if (!strArr.length) {
      const fallback = (pageFillDescription || title || "").trim();
      if (fallback) strArr = [fallback];
    }
    if (!strArr.length) {
      startAI("暂无摘要");
    } else if (strArr.length !== 1) {
      let randomIndex = Math.floor(Math.random() * strArr.length);
      while (randomIndex === lastAiRandomIndex && strArr.length > 1) {
    ...
    setTimeout(() => {
      if (aiTitleRefreshIcon) aiTitleRefreshIcon.style.opacity = "1";
    }, 600);
  }
